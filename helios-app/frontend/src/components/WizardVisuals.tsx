 ...
 * Ported from Healthcare-EPIC-Snowflake-Demo for Helios Energy grid operations.
 *
 * Components:
...
// Accent tokens — aligned with Helios Energy palette
const C = {
  cyan:    '#06b6d4',
  violet:  '#7c3aed',
  dbt:     '#FF694A',
...
export function LineagePanel({
  ...
  return (
    <div className="ops-card flex flex-col" style={{ minHeight: 220 }}>
      <header className="ops-card-header flex items-center justify-between px-5 py-3 border-b" style={{ borderColor: 'var(--hairline)' }}>
        <div className="flex items-center gap-3 flex-wrap min-w-0">
          <div className="eyebrow font-mono" style={{ fontSize: 11, letterSpacing: '0.02em' }}>
            Lineage · building live
          </div>
          <span className="layer-chip cyan" style={{ fontSize: 10, padding: '3px 8px', fontWeight: 700 }}>
            iceberg-resolved
          </span>
        </div>
      ...
      <div className="flex-1 relative" style={{ minHeight: 180, padding: '14px 14px 12px' }}>
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full" role="img" aria-label="Live model lineage">
          ...
          {nodes.map((n) => {
            const fill = n.kind === 'silver' ? '#cfd6e0' : `${C.cyan}`;
            const stroke = n.kind === 'silver' ? '#94a3b8' : (currentStep >= 6 || complete) ? C.bull : C.cyan;
            return (
        ...
        <div className="absolute left-3 bottom-2 right-3 flex items-center justify-between font-mono" style={{ fontSize: 11, color: 'var(--ink-muted)' }}>
          ...
          <span style={{ color: complete ? C.bull : C.cyan }}>
...
export function BuildCompleteSummary({
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-3">
      {panels.map((p) => (
        <div key={p.title} className="ops-card p-4" style={{ borderTop: `3px solid ${p.accent}` }}>
