/*
 * WizardLivePage — dbt-wizard live-build playback for Helios Energy.
 *
...
// Agent accent colors aligned with Helios Energy palette
...
export default function WizardLivePage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(wizardDataUrl('wizard_agents.json')).then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }),
      fetch(wizardDataUrl('wizard_scenario.json')).then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }),
      fetch(wizardDataUrl('wizard_build_script.json')).then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }),
    ]).then(([a, s, b]) => {
      setAgents(a.agents);
      setScenario(s);
      setEvents(b.events);
  ...
  return (
    <div className="wizard-terminal mx-auto max-w-[1640px] px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <span
            ...
            style={{
              ...
              background: 'rgba(6,182,212,0.1)', color: 'var(--cyan)', border: '1px solid rgba(6,182,212,0.35)',
            }}
          >
            <span
              style={{
          ...
          <span className="font-mono" style={{ color: 'var(--ink-muted)', fontSize: 13 }}>
            Step{' '}
            <span style={{ color: 'var(--cyan)', fontWeight: 700 }}>{currentStep}/{totalSteps}</span>
          ...
          >
            <div
              style={{
                ...
                height: '100%',
                background: complete ? 'var(--ok)' : 'var(--cyan)',
      ...
      <div
        className="mb-3 px-4 py-2.5 ops-card border-l-4 flex items-center gap-5 flex-wrap"
        style={{ borderLeftColor: 'var(--cyan)' }}
      >
        <div className="min-w-0 flex-shrink" style={{ flex: '1 1 460px' }}>
          <div className="eyebrow" style={{ fontSize: 10, marginBottom: 2, color: 'var(--cyan)' }}>
            Grid Operations · {scenario.timezone_label} · {scenario.requested_by}
          </div>
          <p
            className="font-medium text-[var(--ink-strong)] leading-snug truncate"
            style={{ fontSize: 16 }}
        ...
        <div className="font-mono text-[var(--ink-muted)] shrink-0" style={{ fontSize: 11 }}>
          Target: <span style={{ color: 'var(--cyan)', fontWeight: 700 }}>{scenario.metric_code}</span>
        </div>
      </div>
            ...
            >
              <div
                ...
                style={{
                  ...
                  color: active ? 'var(--cyan)' : done ? 'var(--ok)' : 'var(--ink-soft)',
                }}
              >
      ...
      >
        ...
        <section
          className="ops-card flex flex-col lg:!h-[calc(100dvh-440px)]"
        ...
        >
          ...
          >
            {visibleNarr.map((m, idx) => {
              ...
              const color = a?.color ?? AGENT_STEP_COLOR[m.e.from] ?? '#06b6d4';
        ...
        <section className="flex flex-col gap-3 lg:!h-[calc(100dvh-440px)]" style={{ minHeight: 'max(60vh, 300px)' }}>
          ...
          {/* SQL panel */}
          <div className="ops-card flex flex-col" style={{ flex: '1.7 1 0' }}>
            <header
              className="px-5 py-3 border-b flex items-center justify-between"
              style={{ borderColor: 'var(--hairline)' }}
            >
              <div className="flex items-center gap-3 flex-wrap min-w-0">
                <div className="eyebrow font-mono" style={{ fontSize: 11, letterSpacing: '0.02em' }}>
                  models/gold/fct_outage_by_substation_tier_quarter.sql
                </div>
                <span
          ...
          {/* YAML panel */}
          <div className="ops-card flex flex-col" style={{ flex: '1 1 0' }}>
            <header
              className="px-5 py-3 border-b flex items-center justify-between"
              style={{ borderColor: 'var(--hairline)' }}
            >
              <div className="flex items-center gap-3 flex-wrap min-w-0">
                <div className="eyebrow font-mono" style={{ fontSize: 11, letterSpacing: '0.02em' }}>
                  models/gold/fct_outage_by_substation_tier_quarter.yml
                </div>
                <span
                  className="layer-chip"
                  style={{
      ...
      <div className="ops-card mt-2 px-3 py-2 flex items-center gap-3">
              ...
              >
                <span
                  style={{
                    ...
                    background: i === 0 ? 'var(--cyan)' : 'var(--ink-soft)',
      ...
      {complete && (
        <div
          className="mt-6 ops-card p-5"
          style={{
            borderLeft: '5px solid var(--ok)',
            ...
            <div className="flex items-baseline gap-3 flex-wrap">
              <div
                ...
                style={{
                  ...
                  background: 'rgba(20,94,54,0.12)', color: 'var(--ok)',
      ...
      <style>{`
        ...
        .wizard-terminal::after {
          content: 'helios-energy/wizard-live · dbt-wizard';
          position: absolute;
          top: 0; left: 0; right: 0;
          height: 28px;
          display: flex;
        ...
        /* Override the card base inside the terminal */
        ...
        .wizard-terminal [style*="color: var(--cyan)"] { color: var(--t-accent) !important; }
        ...
        .wizard-terminal .ops-card[style*="rgba(6,182,212"] {
          background: rgba(34,211,238,0.10) !important;
        }
        .wizard-terminal .ops-card[style*="rgba(20,94,54"] {
