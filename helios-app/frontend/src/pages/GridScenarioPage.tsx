/*
 * GridScenarioPage — Scenario framing page for the dbt-wizard demo.
 *
 * Route: /grid-scenario
 *
 * Shows the VP's question, a T-minus countdown to the PUC filing window,
 * 4-tile KPI grid, upstream-model panel, state-of-world detail,
 * 6-step build path, and a CTA to launch the Live Build.
 *
 * Ported from Healthcare-EPIC-Snowflake-Demo/ClarityScenarioPage — Helios Energy flavor.
 */
...
export default function GridScenarioPage() {
  ...
  useEffect(() => {
    let remaining = 16 * 3600; // 16-hour countdown to PUC filing window
    ...
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <header className="mb-8">
        <div className="flex items-center gap-3 mb-3 flex-wrap">
          <span
            className="status-pill warn inline-flex items-center gap-1.5"
        ...
        <h1 className="text-4xl sm:text-5xl font-semibold leading-[1.05] text-[var(--ink-strong)] tracking-tight">
          {s.timezone_label}.{' '}
          <span className="text-[var(--cyan)]">{s.requested_by}.</span>
        </h1>
        <p className="mt-3 max-w-3xl leading-relaxed text-lg text-[var(--ink-muted)]">
          No <span className="font-mono text-sm">gold.fct_outage_by_substation_tier_quarter</span> exists.
          The West service territory's tier-2 interruption spike is unresolved.
          PUC filing window in 16 hours. Manual build ETA: {s.manual_time_days}.
        ...
        <div
          ...
          style={{ borderLeft: '4px solid var(--cyan)', background: 'rgba(6,182,212,0.05)' }}
        >
          <div className="eyebrow mb-2 text-[var(--cyan)]">The VP of Grid Operations' question</div>
          <p className="text-2xl font-semibold leading-tight text-[var(--ink-strong)]">
            "{s.question}"
          </p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-10">
        <div className="lg:col-span-2 ops-card p-5 relative overflow-hidden">
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            <div>
              <div className="eyebrow">Upstream models available</div>
              <div className="text-xl font-semibold mt-1 text-[var(--ink-strong)]">
                Four signals. Already in the lake.
              </div>
            </div>
            <span className="status-pill ok inline-flex items-center gap-1.5" style={{ fontSize: 11 }}>
              4 of 4
            </span>
          </div>
            ...
            {s.upstream_models.map(u => (
              ...
              >
                <div
                  ...
                  style={{ background: LAYER_COLOR[u.layer] ?? 'var(--cyan)' }}
                />
                <div className="font-mono text-xs pl-1" style={{ color: LAYER_COLOR[u.layer] ?? 'var(--cyan)' }}>
                  {u.layer}
                </div>
        ...
        <div className="ops-card p-5">
          ...
          <dl className="space-y-3 text-sm">
            ...
            <Row k="Service territory" v={s.territory} />
            ...
            <Row k="Lookback window" v={<span className="font-mono">trailing 4 quarters</span>} />
      ...
      {/* 6-step build path */}
      <div
        className="ops-card p-5 mb-8"
        style={{ borderLeft: '4px solid var(--cyan)' }}
      >
        <div className="eyebrow mb-2 text-[var(--cyan)]">The path through six steps</div>
      ...
      {/* CTA */}
      <div className="flex flex-col sm:flex-row gap-3 items-center justify-between ops-card p-5">
        <div>
          <div className="text-2xl font-semibold text-[var(--ink-strong)]">
        ...
        <Link
          ...
          className="inline-flex items-center gap-2 rounded-md text-[var(--navy-deep)] font-semibold px-6 py-4 whitespace-nowrap hover:opacity-95 transition-opacity"
...
const STEPS = [
  { title: 'Discovery',             who: 'Explorer',     tools: 'status, search',        color: 'var(--cyan)' },
  ...
  { title: 'Test Authoring',        who: 'Verification', tools: 'describe, dbt_show',    color: 'var(--ok)' },
  { title: 'Materialization',       who: 'Worker + Ver', tools: 'dbt_run, lineage',      color: 'var(--ok)' },
];
...
function KpiTile({
  ...
  return (
    <div
      className="ops-card p-5 relative overflow-hidden"
      style={{ borderLeft: `4px solid ${tone}` }}
    >
      <div className="eyebrow mb-2">{label}</div>
      <div
        className="text-3xl font-semibold tracking-tight tabular mono"
        style={{ color: tone }}
      >
        {value}
      </div>
